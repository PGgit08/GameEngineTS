...
export class NoZoomBehavior extends Behavior {
    ...
    public override update(): void {
        this.transform.scale = vec2.fromValues(
            SceneManager.getInstance().currentScene.currentCamera.size * this.owner.transform.scale[0],
            SceneManager.getInstance().currentScene.currentCamera.size * this.owner.transform.scale[1]
        );
    }
}